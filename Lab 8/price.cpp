...
float findHighestPrice(PriceType, int, int); // finds highest price
float findLowestPrice(PriceType, int, int); // finds lowest price
...
void printPrices(PriceType table, int numOfRows, int numOfCols)
{
	...
	for (int row = 0; row < numOfRows; row++)
	{
		for (int col = 0; col < numOfCols; col++)
		{
            ...
            cout << setw(7) << std::left << table[row][col];
            if (col == numOfCols-1) {
                cout << endl;
            }
		}
	}
    cout << "the highest price is: " << findHighestPrice(table, numOfRows, numOfCols) << endl;
    cout << "the lowest price is: " << findLowestPrice(table, numOfRows, numOfCols) << endl;
}
